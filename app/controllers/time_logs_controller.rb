class TimeLogsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @time_log.save
        ...
        format.xml  { render :xml => @time_log, :status => :created,
			:location => @time_log }
      else
        format.html { render :action => "new" }
        format.xml  { render :xml => @time_log.errors,
    ...
    respond_to do |format|
      if @time_log.update_attributes(params[:time_log])
        ...
        format.html { redirect_to(:action => "new",
			:project_id => @time_log.task_type.project.id) }
        format.xml  { head :ok }
      else
  ...
  private
  def setup_variables
	@time_logs = TimeLog.find :all, :conditions => {:user_id => @current_user.id},
